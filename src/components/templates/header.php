<?php

$header = new PageComponent(
    props: $_PROPS,
    mounted: function(object &$values, array $props): void {
        $db = FlashNewsDB::getPdo();
        $usersDao = new UsersDAO($db);
        
        $values->user = $usersDao->getUserByEmail('test@test.com');
    }
);

$user = $header->values->user;
$userUrl = '/user/index.php?id='. $user['id'];

?>

<header class="flex justify-center mt-10 mb-3">
    <div class="flex justify-between w-9/12 flex-wrap">
        <h1 class="text-6xl text-gray-800 font-bold"><i class="fa-solid fa-bolt-lightning"></i> Flash News</h1>
        <div class="flex items-center text-gray-700">
            <a href="/actions/logout.php" class="hover:bg-gray-200 rounded-lg p-2 mr-4">
                <i class="fa-solid fa-right-from-bracket"></i> ログアウト
            </a>
            <a href="<?=$userUrl ?>" class="flex items-center p-2 hover:bg-gray-200 rounded-lg border border-gray-300">
                <img class="w-7 h-7 rounded-full object-cover mr-1" src="/img/news.jpg" alt="user image">
                <p class="text-xl font-bold">@<?=$user['username'] ?></p>
            </a>
        </div>
    </div>
</header>
<hr class="ml-3 mr-3 mb-5"/>
